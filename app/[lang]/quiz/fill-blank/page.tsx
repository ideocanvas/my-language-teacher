...
export default function FillBlankQuizPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  useEffect(() => {
    if (!currentSession && dailyReview && dailyReview.dueWords.length > 0) {
      startQuiz("fill-blank", dailyReview.dueWords);
  ...
  const getBlankSentence = useCallback(() => {
    if (!currentWord) return null;
    
    // Use example sentence if available
    if (currentWord.exampleSentences.length > 0) {
      const sentence = currentWord.exampleSentences[0];
      ...
      // If word wasn't found in sentence, create a simple prompt
      if (blanked === sentence) {
        return {
          text: `_____ means "${currentWord.translation}"`,
          hint: currentWord.definitions[0] || "",
        };
      }
    ...
    // Fallback: create a simple definition-based prompt
    return {
      text: `_____ means "${currentWord.translation}"`,
      hint: currentWord.definitions[0] || "",
    };
  ...
  const blankSentence = getBlankSentence();
  const isCorrect = checkAnswer();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Question */}
        {currentWord && blankSentence && (
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 mb-6">
            <div className="text-center mb-8">
              <p className="text-sm text-gray-500 mb-4">{t("quiz.fillBlank.title")}</p>
              
              {/* Sentence with blank */}
              <div className="bg-gray-50 rounded-xl p-6 mb-6">
                <p className="text-xl text-gray-900 leading-relaxed">{blankSentence.text}</p>
              </div>
